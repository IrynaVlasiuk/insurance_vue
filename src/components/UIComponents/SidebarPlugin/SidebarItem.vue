    ...
    <slot name="title" v-if="children.length === 0 && !$slots.default && link.path">
      ...
      >
        <template v-if="addLink">
          <span class="sidebar-mini"><i :class="'fa fa-'+link.fa_icon"></i>&nbsp;</span>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      if(this.link.name =='sidebar.map'){
        if(!this.$store.getters.isAuthAdmin) {
          //this.isVisible = false
        }
      }
      if(this.link.name =='sidebar.supervisor'){
          if(!this.$store.getters.isAuthAdmin) {
              this.isVisible = false
          }
      }
      if(this.$store.getters.isAgent){
          if(this.link.name == 'sidebar.map'
            || this.link.name == 'sidebar.documents'
            || this.link.name == 'sidebar.supervisor'
            || this.link.name == 'sidebar.administration'
            || this.link.name == 'sidebar.users'){
              this.isVisible = false;
          }
      }
    },
